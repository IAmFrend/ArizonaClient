...
namespace ArizonaClient
{
    class Configuration_class
    {
        ...
        public void SQL_Server_Configuration_Get()
        {
            ...
            RegistryKey key = registry.CreateSubKey("ArizonaClient");
            try
            {
        ...
        public void SQL_Data_Base_Collection()
        {
            ...
            //SqlCommand command = new SqlCommand("select name from sys.databases where name not in ('master','tempdb','model','msdb') and name like 'ArizonaDatabase%'", connection);
