            ...
            if (ApplicationDeployment.IsNetworkDeployed)
            {
                ...
                try
                {
                    if (info.UpdateAvailable)
                    {
                        ...
                        {
                            DialogResult dr = MessageBox.Show("Доступно обновление. Новая (последняя доступная) версия: "+info.AvailableVersion.ToString()+". Вы хотите его загрузить?", "ArizonaClient", MessageBoxButtons.OKCancel);
